...
fn rpc_git_clone(req string, mut app core.App) string {
	url := core.get_arg(req, 0) or { return '{"error": "Missing repo URL"}' }
	os.mkdir_all('workspace') or { return '{"error": "Failed to create workspace: $err.msg()"}' }
	cmd := 'cd workspace && git clone --depth=1 ' + url + ' 2>&1'
	res := os.execute(cmd)
	if res.exit_code != 0 {
		return json.encode({ 'error': res.output.trim_space() })
	}
...
fn rpc_git_remove(req string, mut app core.App) string {
	name := core.get_arg(req, 0) or { return '{"error": "Missing repo name"}' }
	target := os.join_path('workspace', name)
	trash_dir := 'workspace_trash'
	trash_target := os.join_path(trash_dir, name)
	if !os.is_dir(target) {
		return json.encode({ 'error': 'Repository not found: $name' })
	}
	os.mkdir_all(trash_dir) or { return json.encode({ 'error': 'Failed to create trash: $err.msg()' }) }
	if os.is_dir(trash_target) {
		os.rmdir_all(trash_target) or {}
	}
	os.mv(target, trash_target) or { return json.encode({ 'error': 'Failed to remove: $err.msg()' }) }
...
fn rpc_git_restore(req string, mut app core.App) string {
	...
	target := os.join_path('workspace', name)
	if !os.is_dir(trash_target) {
		return json.encode({ 'error': 'Repository not found in trash: $name' })
	}
	os.mkdir_all('workspace') or {}
	if os.is_dir(target) {
		return json.encode({ 'error': 'Cannot restore, repository already exists: $name' })
	}
	os.mv(trash_target, target) or { return json.encode({ 'error': 'Failed to restore: $err.msg()' }) }
	return json.encode({ 'success': true })
}
